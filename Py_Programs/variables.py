...
#Variables are names that hold values that you can use later in your code,In python, you create variables by assigning a valueto it using the "=" operator.
#Key points to take home are -No need to declare the typeeg integer,string before hand
...
#Integer
#Explanation: They are whole numbers(that's no decimals) they are usefull for counts, indices, or any dat that does not need fractional precision
num_rows = 100
...
print("Your average salary is $.", average_salary)
...
#Dictionaries
#They are unordered collection of key of key value pairs enclosed in curly braces{}.They are great for structured data where you need to look up values by a key  like mini database
...
#Mini exercise
#Create variables for a dataset's name(string), row  count(integer), and average avalue(integer)
#Solution
